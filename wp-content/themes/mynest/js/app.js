...
/* WEBPACK VAR INJECTION */(function(jQuery) {jQuery.noConflict(); // Reverts '$' variable back to other JS libraries
/**
...
jQuery(document).ready(function ($) {
    ...
    var toastalert = $('<div class="toast toast__warning" role="alert" aria-live="assertive" data-animation="true" aria-atomic="true" data-autohide="true" data-delay="10000">\
        ...
        <div class="toast-body warning__body">' + toastbody + "</div>\
    </div>");
            ...
            if (func["ID"] == identremove) {
                ...
                for (var _i in proflist) {
                    if (proflist.hasOwnProperty(_i)) {
                        ...
                        namesprofs = namesprofs.map(JSON.stringify).reverse().filter(function (item, index, arr) {
                            return arr.indexOf(item, index + 1) === -1;
                        }).reverse().map(JSON.parse);
                        idsProfs = idsProfs.map(JSON.stringify).reverse().filter(function (item, index, arr) {
                            return arr.indexOf(item, index + 1) === -1;
                        }).reverse().map(JSON.parse);
                    }
                }
    ...
    $(document).on("click", ".button--contratar", function () {
        var profinfo = {
            name: $(this).parent().find(".item__title").text(),
            ID: $(this).data("id")
        };

        proflist.push(profinfo);
        proflist = proflist.map(JSON.stringify).reverse().filter(function (item, index, arr) {
            return arr.indexOf(item, index + 1) === -1;
        }).reverse().map(JSON.parse);
        $(".list__profsadd").html("");
        $(".num__personal").html(proflist.length);
        ...
        for (var i in proflist) {
            if (proflist.hasOwnProperty(i)) {
                ...
                $(".list__profsadd").append('<div class="list__profsadd-item"><i class="remove__person" data-delfun="' + element["ID"] + '">x</i>' + element["name"] + "</div>");
    ...
    function submitpost(typ, atrb, prc, anch) {
        ...
        $.post(js_global.xhr_url, data, function (data, textStatus, jqXHR) {
            ...
            if (data.results[0].erros) {
                if (data.results[0].field == "cep") {
                    ...
                    $(toastalert).find(".warning__body").html(data.results[0].message);
                    ...
                    var _data$results$ = data.results[0],
                        message = _data$results$.message,
                        terms = _data$results$.terms;


                    toastbody = message;
                    $(toastalert).find(".warning__body").html(message);
                    $(".toast__wrapper").prepend(toastalert);
                    $(".toast__warning").toast("show");
                    //shown.bs.toast
                    $(".toast__warning").on("hidden.bs.toast", function () {
                    ...
                    $(".post__type-" + typ).find(".list__profissionais").html('<div style="color: #000000; font-size: 35px; margin: 20px auto;">' + message + "</div>");
                    return false;
                }
            } else {
                ...
                if (categorias.length > 0 && !atrb) {
                    $(".select__require").html("");
                    for (var i in categorias) {
                        if (categorias.hasOwnProperty(i)) {
                            var element = categorias[i];
                            $(".select__require").append('<li class="" data-rf="refresh" data-atrib="' + element["slug"] + '">' + element["name"] + "</li>");
                        }
                    }
                }
                ...
                for (var i in results) {
                    if (results.hasOwnProperty(i)) {
                        ...
                        var formatprice = price > 999 ? price.substring(0, 1) + "." + price.substring(1, 100) : price;
                        var clsstt = "";
                        if (!typ) {
                            $(".post__type-baba").find(".list__profissionais").append('\
                            <li class="item__list">\
                                <figure>' + thumbnail + '</figure>\
                                <span class="item__attr">' + atributo + '</span>\
                                <span class="item__title">' + title + '</span>\
                                <span class="item__desc">' + description + '</span>\
                                <span class="item__salario"><strong>Prentensão salarial</strong>R$' + formatprice + ',00</span>\
                                <span class="button--contratar" data-id="' + ID + '">Contratar</span>\
                            </li>\
                            ');
                        } else {
                            $(".post__type-" + typ).find(".list__profissionais").append('\
                              <li class="item__list">\
                                  <figure>' + thumbnail + '</figure>\
                                  <span class="item__attr">' + atributo + '</span>\
                                  <span class="item__title">' + title + '</span>\
                ...
                $("#cepuser").parents(".modal__container").addClass("close");
    ...
    $(document).on("click", ".select__wrapper > button", function () {
        atrib = $(this).parent().find(".active__aba").data("atrib");
        price = "";
        $(".prices__container").addClass("active");
        submitpost(type, atrib, price, {
            target: ".select__require",
            offset: -150
        });
    });
    ...
    /*  $(document).on("click", ".select__require > li:first-child", function() {
         $(this).parent().toggleClass('expand')
     })
      $(document).on("click", ".select__require > li:not(:first-child)", function() {
         $(this).parent().toggleClass('expand')
     }) */

    $(document).on("click", ".select__require > li", function () {
        atrib = $(this).data("atrib");
        $(this).parent().toggleClass('expand');
        price = "";
        $(".prices__container").addClass("active");
        $(".select__require > li").removeClass("active__aba");
        $(this).addClass("active__aba");
        var elm = $(this).clone();
        $(this).remove();
        $('.select__require').prepend(elm);
        /* submitpost(type, atrib, price, {
            target: ".select__require",
            offset: -150
        }); */
    ...
    $(document).on("click", ".list__price > li", function () {
        price = [parseInt($(this).data("priceIn")), parseInt($(this).data("priceOut"))];
    ...
    $(document).on("click", ".modal__close", function () {
        $(this).parents(".modal__container").addClass("close");
    });
    ...
    $(document).on("click", ".modal__footer > button", function (e) {
        if ($("#cepuser").val()) {
            cepuser = $("#cepuser").val();
            var elmts = $(this);
            if (submitpost(type, atrib, price, {
                target: ".list__service",
                offset: 60
            }) === true) {
                $(document).find(".active__aba").trigger("click");
                elmts.parents(".modal__container").addClass("close");
            }
        } else {
            toastbody = "Informe o CEP para a busca!";
            $(toastalert).find(".warning__body").html("Informe o CEP para a busca!");
            $(".toast__wrapper").prepend(toastalert);
            $(".toast__warning").toast("show");
            //shown.bs.toast
            $(".toast__warning").on("hidden.bs.toast", function () {
        ...
        if ($("list__service:eq(0)").hasClass("active__aba")) {} else {
            $(".domestics").hide();
        }
        e.preventDefault();
    });
    ...
    $(document).on("click", ".idade__item", function () {
        $(this).parent().find(".idade__item").removeClass("selected");
