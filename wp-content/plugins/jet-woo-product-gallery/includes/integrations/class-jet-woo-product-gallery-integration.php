...
if ( ! class_exists( 'Jet_Woo_Product_Gallery_Integration' ) ) {
	...
	class Jet_Woo_Product_Gallery_Integration {
		...
		public function init() {
			...
			// Photoswipe styles in quick view popup
			add_filter( 'jet-popup/widgets/photoswipe-styles', array( $this, 'get_photoswipe_styles'), 10, 1 );
		...
		/**
		 * Get photoswipe gallery styles
		 *
		 * @param  string $styles photoswipe styles
		 * @return string
		 */
		public function get_photoswipe_styles( $styles ) {
			global $woocommerce;

			$photoswipe              = str_replace( 'background-image:url(default-skin/', 'background-image:url(' . $woocommerce->plugin_url() . '/assets/css/photoswipe/default-skin/', file_get_contents( $woocommerce->plugin_url() . '/assets/css/photoswipe/photoswipe.min.css' ) );
			$photoswipe_default_skin = str_replace( 'background-image:url(', 'background-image:url(' . $woocommerce->plugin_url() . '/assets/css/photoswipe/default-skin/', file_get_contents( $woocommerce->plugin_url() . '/assets/css/photoswipe/default-skin/default-skin.min.css' ) );
			$photoswipe_default_skin = str_replace( 'background:url(', 'background:url(' . $woocommerce->plugin_url() . '/assets/css/photoswipe/default-skin/', file_get_contents( $woocommerce->plugin_url() . '/assets/css/photoswipe/default-skin/default-skin.min.css' ) );

			$styles = $photoswipe . $photoswipe_default_skin;
